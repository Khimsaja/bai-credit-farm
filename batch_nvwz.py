#!/usr/bin/env python3
"""
B.AI Claim Bonus — v2 with better timeout & retry handling.
Patched Turnstile + 2captcha + browser UI claim.
"""
...
RESULTS_FILE = os.environ.get("B_AI_RESULTS_FILE", "results.json")
PROFILE_BASE = os.environ.get("B_AI_PROFILE_BASE", "/tmp/bai_profile")
TOTAL_ACCOUNTS = int(os.environ.get("B_AI_TOTAL_ACCOUNTS", "250"))
ACCOUNT_PREFIX = os.environ.get("B_AI_ACCOUNT_PREFIX", "user")
KEY_DIR = os.environ.get("B_AI_KEY_DIR", ".")

def save_key(account_id, key):
    """Save key to file and return it."""
...
def is_valid_full_key(val):
    """Check if value is a full (unmasked) API key."""
...
def scrape_key_from_modal(page, account_id):
    """Try multiple methods to extract full key from the success modal.
    
    KEY FINDING: In the success modal (after creating a key), B.AI shows 
    the FULL unmasked key in an input[readonly] field. The key is plaintext 
    — no masking. Masking only happens in the key list table and API responses.
    """
    import re
    
    # Method 1: Read input[readonly] value directly
    try:
    ...
    # Method 2: Check all text content on page for sk- pattern
    try:
    ...
    try:
        ...
        if copy_btn.is_visible(timeout=3000):
            ...
            # Fallback: re-check readonly (might have been updated)
            try:
...
def extract_key_via_clipboard(page, account_id):
    """Extract API key after navigating to /key page.
    
    Full key is visible in the success modal's input[readonly] field —
    no masking there! Only the key list table and API responses are masked.
    ...
    # Final fallback
    try:
        page.screenshot(path=f'/tmp/bai_keyfail_{account_id}.png', full_page=True)
    except:
        pass
    try:
...
def log(msg):
    ...
    with open("/root/bai-keys/claim_nvwz.log", "a") as f:
...
def claim_account(account_id):
    ...
    try:
        with sync_playwright() as pw:
            ...
            # 🎯 CLIPBOARD INTERCEPT: Patch BOTH writeText AND execCommand
            # B.AI masks API keys at server level — Copy button is the only way to get full key
            page.add_init_script("""
// Method 1: Patch navigator.clipboard.writeText
...
// Method 2: Patch document.execCommand for older clipboard API
...
""")
            ...
            # Cek apakah udah login atau butuh login flow
            ...
            # 3️⃣ Navigate to chat.b.ai — long timeout since OAuth redirects
            ...
            # 7️⃣ Open bonus modal & claim
            try:
                page.locator('button:has-text("Claim Free Credits")').first.click(timeout=10000)
                time.sleep(5)
            except:
                # Already claimed — skip key extraction (previous runs already created keys,
                # and accounts with 3 keys have no "Create API key" button)
                log(f"{account_id}: ⚠️ Claim button not found (already claimed), skipping key extraction")
            ...
            # Take screenshot
            try:
                page.screenshot(path=f"/tmp/claim_{account_id}.png")
            except:
                pass
        ...
        if profile_path.exists():
            shutil.rmtree(profile_path)
        # Only kill Chrome from this worker's profile — exact match, not substring!
        subprocess.run(['pkill', '-9', '-f', f'chrome.*{PROFILE_BASE}_'], capture_output=True)
...
def main():
    ...
    log(f"📊 Existing: {len(results)} accounts, ✅{existing_success} ❌{existing_errors}")
    
    for i in range(1, TOTAL_ACCOUNTS + 1):
        acct = f"{ACCOUNT_PREFIX}{i}"
        
        # Skip if already success AND has key file
        ...
        # Re-process: skip if only claim success but no key
        if results.get(acct, {}).get('status') == 'success' and not has_key:
            log(f"🔄 {acct}: Re-claiming for key extraction (had claim, no key)")
            # Reduce delay for re-processing
            delay = random.randint(5, 15)
        else:
            # New account — random delay between accounts (20-60s) to avoid rate limiting
    ...
    log(f"🏁 DONE: ✅{total_ok} ❌{total_err} | Total: {len(results)}")
